...
Automatically categorizes detections into confidence levels:

| Level | Criteria | Recommended Action |
|-------|----------|-------------------|
| **High** | Both databases + reads ≥100 | Direct reporting |
| **High-Low** | Both databases + reads <100 | Cautious acceptance |
| **Medium** | Single database + reads ≥100 | Validation required |
| **Medium-Low** | Single database + 50≤reads<100 | Strong validation needed |
| **Low** | Single database + reads <50 | Likely false positive |
...
→ Action: BLAST validation, sequence assembly
```

#### Low Confidence (Cautious interpretation)
```
✗ Detected in only one database
✗ Direct reads < 50
✗ Likely false positive or contamination
→ Action: Do not report unless specifically relevant
```
...
### For Candidate Viruses (Medium Confidence)
...
High-confidence viruses were defined as those detected in both databases 
with ≥100 direct read assignments. Medium-confidence viruses were 
detected in a single database with ≥100 reads and were subjected to 
further validation. Low-confidence detections (<50 reads) were excluded 
from analysis to minimize false positives.
...
Additionally, N medium-confidence candidate viruses were identified in 
a single database (Supplementary Table S2), potentially representing 
rare or recently discovered viruses requiring further validation.
```
