...
if [ $? -eq 0 ]; then
    ...
    if [ -d "results" ]; then
        ...
        if [ -d "results/assembly_megahit" ]; then
            echo "  ✅ MEGAHIT assembly contigs: results/assembly_megahit/"
            MEGAHIT_CONTIGS=$(find results/assembly_megahit -name "*_contigs.fa" | wc -l)
            echo "     - Generated $MEGAHIT_CONTIGS contig files"
        fi
        
        if [ -d "results/assembly_spades" ]; then
            echo "  ✅ SPAdes assembly contigs: results/assembly_spades/"
            SPADES_CONTIGS=$(find results/assembly_spades -name "*_contigs.fa" | wc -l)
        ...
        find results/assembly_* -name "*_contigs.fa" 2>/dev/null | head -10
        echo ""
