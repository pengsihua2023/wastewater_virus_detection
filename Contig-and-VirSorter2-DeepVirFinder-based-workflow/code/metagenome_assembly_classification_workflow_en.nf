...
process COMPARE_ASSEMBLERS {
    ...
    # Extract viral sequences (using correct column name)
    megahit_seqs = set(megahit_df['sequence_name'].tolist()) if 'sequence_name' in megahit_df.columns else set()
    spades_seqs = set(spades_df['sequence_name'].tolist()) if 'sequence_name' in spades_df.columns else set()
